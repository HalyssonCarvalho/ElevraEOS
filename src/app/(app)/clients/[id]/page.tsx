    ...
    <div className="flex flex-col gap-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <MetricCard
          ...
          value={formatNumber(overview.current.leads)}
          previousValue={overview.previous ? formatNumber(overview.previous.leads) : null}
        />
        ...
        <MetricCard
          label="Receita"
          value={formatCurrency(overview.current.revenue, { compact: true })}
          previousValue={overview.previous ? formatCurrency(overview.previous.revenue, { compact: true }) : null}
          variation={overview.revenueGrowth}
        />
        ...
        <MetricCard
          ...
          value={overview.conversionRate === null ? "—" : formatPercent(overview.conversionRate)}
        />
        <MetricCard
          label="Ticket médio"
          value={overview.averageTicket === null ? "—" : formatCurrency(overview.averageTicket)}
        />
        ...
        <MetricCard
          ...
          value={extras ? `${extras.googleRating.toFixed(1)} ★ (${extras.googleReviewsCount})` : "—"}
        />
        <MetricCard
          label="Tempo médio de resposta"
          value={extras ? `${extras.avgResponseMinutes} min` : "—"}
        />
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        ...
        <Card>
          ...
          <CardContent className="pt-3 flex flex-col gap-4">
            ...
            <GoalBar
              label="Receita"
              progress={overview.revenueGoalProgress}
              goalLabel={`Meta: ${formatCurrency(client.monthly_revenue_goal, { compact: true })}`}
            />
          </CardContent>
        </Card>
      </div>
